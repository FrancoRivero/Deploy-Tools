...
install_virtualbox (){
    wget -q https://www.virtualbox.org/download/oracle_vbox_2016.asc -O- | sudo apt-key add - &>> $LOG_FILE
    wget -q https://www.virtualbox.org/download/oracle_vbox.asc -O- | sudo apt-key add - &>> $LOG_FILE
    sudo add-apt-repository "deb [arch=amd64] http://download.virtualbox.org/virtualbox/debian $(lsb_release -cs) contrib" &>> $LOG_FILE
    sudo apt update &>> $LOG_FILE
    sudo apt-get install -y virtualbox-6.1 &>> $LOG_FILE
}
